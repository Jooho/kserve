...
**Checkpoint file**: `.release-checkpoint.json` in the repo root

**Save checkpoint** (write this file before any long-running step):
```bash
cat > .release-checkpoint.json << EOF
{
  "version": "{VERSION}",
...
5. **Save checkpoint** after PR created:
   ```bash
   # Update .release-checkpoint.json with bump_pr number and phase
