	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			result, err := Build(Input{
				Namespace: "team", WorkloadName: "model", RuntimeImage: tc.image,
				ModelURIHash: modelHash, TensorParallelSize: "2",
			})
...
func TestBuildRequiresModelURIHash(t *testing.T) {
	_, err := Build(Input{
		Namespace: "team", WorkloadName: "model",
...
func TestBuildIncludesRuntimeFactorsOnlyInCompatibilityFootprint(t *testing.T) {
	base := Input{
		Namespace:          "team-a",
		WorkloadName:       "model-a",
		RuntimeImage:       "registry.example/vllm@sha256:" + strings.Repeat("b", 64),
	...
	changed := base
	changed.RuntimeFactors = map[string]string{DTypeFactor: "float16"}
