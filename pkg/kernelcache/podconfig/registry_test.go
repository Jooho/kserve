...
func TestRegistryProjection(t *testing.T) {
	for _, capture := range []bool{false, true} {
		...
		if capture {
			...
			require.NoError(t, ApplyRegistry(&pod, &container, cfg))
		}
...
func TestRegistryCAWithoutAuthentication(t *testing.T) {
	cfg := v1beta1.KernelCacheRegistryConfig{Auth: v1beta1.KernelCacheRegistryAuth{Type: "none"}, CAConfigMapRef: &v1beta1.KernelCacheConfigMapKeyRef{Name: "ca", Key: "bundle"}}
	pod := corev1.PodSpec{}
	...
	require.Error(t, ApplyRegistry(&pod, &container, cfg))
}
