...
// ApplyCaptureReporter mounts required access that only patches capture status.
...
// ApplyCaptureReporterRequired is retained for source compatibility. Reporter
// access is always required for the current capture flow.
func ApplyCaptureReporterRequired(pod *corev1.PodSpec, container *corev1.Container, secretName string) error {
	return ApplyCaptureReporter(pod, container, secretName)
}
