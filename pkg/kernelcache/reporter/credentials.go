...
func (c *Credentials) issue(ctx context.Context, pod *corev1.Pod, serviceAccountName, requestedSecretName string, owner *metav1.OwnerReference) (*corev1.Secret, error) {
	...
	if apierrors.IsNotFound(err) {
		secretOwner := owner
		if secretOwner == nil {
			secretOwner = metav1.NewControllerRef(live, corev1.SchemeGroupVersion.WithKind("Pod"))
			secretOwner.BlockOwnerDeletion = boolPtr(false)
		}
		secret, err = secrets.Create(ctx, &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				...
				OwnerReferences: []metav1.OwnerReference{*secretOwner},
			},
