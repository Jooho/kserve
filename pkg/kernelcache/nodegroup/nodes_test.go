...
func TestMatchingGroups(t *testing.T) {
	...
	groups := []v1alpha1.KernelCacheNodeGroup{
		...
		{ObjectMeta: metav1.ObjectMeta{Name: "gpu-pvc"}, Spec: v1alpha1.KernelCacheNodeGroupSpec{
			NodeSelector: map[string]string{"role": "gpu", "zone": "east"},
	...
	ociMatches := MatchingGroups(node, groups, v1alpha1.KernelCacheMountTypeOCI)
	if len(ociMatches) != 2 || ociMatches[0].Name != "gpu-oci" || ociMatches[1].Name != "gpu-pvc" {
		t.Fatalf("unexpected OCI matches: %#v", ociMatches)
	}
	pvcMatches := MatchingGroups(node, groups, v1alpha1.KernelCacheMountTypePVC)
	if len(pvcMatches) != 1 || pvcMatches[0].Name != "gpu-pvc" {
		t.Fatalf("unexpected PVC matches: %#v", pvcMatches)
	}
}
