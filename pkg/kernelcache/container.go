...
// ResolveRuntimeContainerName resolves an optional cache path container name.
...
// ResolveContainerPath resolves an optional cache path for a runtime container.
// An explicit path takes precedence over the runtime environment and fallback.
...
// ResolveOCIPath returns an OCI cache path supported by the current MCV image
// format. The value remains fixed until MCV supports caller-provided paths.
func ResolveOCIPath(requested string) (string, error) {
	// TODO: Make OCIPath configurable when MCV supports caller-provided OCI paths.
	if requested == "" {
