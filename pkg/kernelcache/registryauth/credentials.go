...
// Credentials is called only after the caller authenticates and authorizes the capture session.
...
func (c *Credentials) Issue(ctx context.Context, pod *corev1.Pod, cfg v1beta1.KernelCacheRegistryConfig) (*corev1.Secret, error) {
	...
	if live.UID != pod.UID || live.UID == "" || live.DeletionTimestamp != nil || live.Annotations[InjectedAnnotation] != "true" {
