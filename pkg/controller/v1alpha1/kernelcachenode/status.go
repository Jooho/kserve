...
func (r *KernelCacheNodeReconciler) reconcileStatus(
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		podsUsing := 0
		if validateImages {
			if kernelCacheNode.Status.Counts != nil {
				podsUsing = kernelCacheNode.Status.Counts.TotalPodsUsing
			}
			nodeReady, err := r.validateCacheImages(ctx, kernelCacheNode)
			if err != nil {
				return fmt.Errorf("validate KernelCache images for node %q: %w", r.NodeName, err)
			}
			if !nodeReady {
				podsUsing = 0
			}
		} else {
			var needsImageValidation bool
			var err error
			...
			if err := r.updateCacheStatuses(ctx, kernelCacheNode, config, needsImageValidation); err != nil {
...
// validateCacheImages validates the image references already recorded in KCN status.
// It intentionally does not list KernelCaches or inspect preparation Jobs.
func (r *KernelCacheNodeReconciler) validateCacheImages(
	ctx context.Context,
	kernelCacheNode *v1alpha1.KernelCacheNode,
) (bool, error) {
	...
	if err := r.Get(ctx, client.ObjectKey{Name: r.NodeName}, node); err != nil {
		return false, fmt.Errorf("get Node %q for image validation: %w", r.NodeName, err)
	}
