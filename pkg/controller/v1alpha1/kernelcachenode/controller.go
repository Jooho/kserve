...
func (r *KernelCacheNodeReconciler) reconcileStatus(ctx context.Context, config *v1beta1.KernelCacheConfig) error {
...
func (r *KernelCacheNodeReconciler) updateCacheStatuses(
	...
	for cacheKey, cacheInfo := range kernelCacheNode.Status.CacheStatus {
		...
		if job == nil && cacheInfo.State == v1alpha1.KernelCacheNodePreparationStateReady && node == nil {
			node = &corev1.Node{}
			if err := r.Get(ctx, client.ObjectKey{Name: r.NodeName}, node); err != nil {
				return err
			}
		}
		state, message := cachePreparationState(job, cacheInfo, podFailureMessage, node)
...
func cachePreparationState(
	...
	state, message := preparationState(job, cacheInfo.State, podFailureMessage)
	if job == nil && cacheInfo.State == v1alpha1.KernelCacheNodePreparationStateReady &&
		!nodeContainsImage(node, cacheInfo.ImageReference) {
