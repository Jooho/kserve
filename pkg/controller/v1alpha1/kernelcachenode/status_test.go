...
func TestValidateCacheImagesUsesKCNStatus(t *testing.T) {
	const (
		nodeName = "gpu-node-1"
		imageOne = "registry.example.com/cache-one@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
		imageTwo = "registry.example.com/cache-two@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
	)
	...
	node := &corev1.Node{
		...
		Status: corev1.NodeStatus{
			...
			Images:     []corev1.ContainerImage{{Names: []string{imageOne}}},
		},
	}
	kernelCacheNode := &v1alpha1.KernelCacheNode{
		ObjectMeta: metav1.ObjectMeta{Name: nodeName},
		Status: v1alpha1.KernelCacheNodeStatus{
			CacheStatus: map[string]v1alpha1.KernelCacheNodeCacheInfo{
				"team/cache-one": {
					ImageReference: imageOne,
					State:          v1alpha1.KernelCacheNodePreparationStatePending,
				},
				"team/cache-two": {
					ImageReference: imageTwo,
	...
	nodeReady, err := reconciler.validateCacheImages(t.Context(), kernelCacheNode)
	if err != nil {
		t.Fatal(err)
	}
	if !nodeReady {
		t.Fatal("expected the node to be ready")
	}
	if got := kernelCacheNode.Status.CacheStatus["team/cache-one"].State; got != v1alpha1.KernelCacheNodePreparationStateReady {
		t.Fatalf("expected image-one cache to be Ready, got %q", got)
	}
	if got := kernelCacheNode.Status.CacheStatus["team/cache-two"].State; got != v1alpha1.KernelCacheNodePreparationStatePending {
		t.Fatalf("expected image-two cache to be Pending, got %q", got)
	}
}
