...
func TestPeriodicImageValidationUsesKCNStatusWithoutKernelCacheOrJob(t *testing.T) {
	const (
		nodeName       = "gpu-node-1"
		cacheNamespace = "team"
		cacheName      = "cache"
	...
	scheme := runtime.NewScheme()
	for _, add := range []func(*runtime.Scheme) error{v1alpha1.AddToScheme, v1beta1.AddToScheme, corev1.AddToScheme} {
		if err := add(scheme); err != nil {
			t.Fatal(err)
		}
	}
	...
	kernelCacheNode := &v1alpha1.KernelCacheNode{
		...
		Status: v1alpha1.KernelCacheNodeStatus{
			...
			Counts: &v1alpha1.KernelCacheNodeCounts{TotalPodsUsing: 3},
		},
	}
	...
	k8sClient := fake.NewClientBuilder().
		WithScheme(scheme).
		WithStatusSubresource(kernelCacheNode).
		WithObjects(node, kernelCacheNode).
		Build()
	reconciler := &KernelCacheNodeReconciler{Client: k8sClient, NodeName: nodeName}

	if err := reconciler.reconcileStatus(t.Context(), &v1beta1.KernelCacheConfig{}, true); err != nil {
		t.Fatal(err)
	}
	...
	if got := updated.Status.Counts.TotalPodsUsing; got != 3 {
		t.Fatalf("expected TotalPodsUsing to be preserved, got %d", got)
	}
}
