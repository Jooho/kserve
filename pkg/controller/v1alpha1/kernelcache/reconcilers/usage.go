...
// consumerPodPredicate accepts Pods that declare KernelCache usage and relevant lifecycle changes.
// It filters out unrelated Pods before the usage handler computes KC reconcile requests.
func consumerPodPredicate() predicate.Predicate {
	return predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			return hasKernelCacheUsageAnnotation(e.Object)
		},
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if !hasKernelCacheUsageAnnotation(oldPod) && !hasKernelCacheUsageAnnotation(newPod) {
				return false
			}
...
// enqueueKCOnPodUsageChange maps a serving Pod's KernelCache usage annotation to its KC.
// Pod lifecycle or node changes can change KC usage, so the referenced KC must refresh its usage status.
// Invalid, cross-namespace, or missing usage references are ignored.
func (r *KernelCacheReconciler) enqueueKCOnPodUsageChange(_ context.Context, obj client.Object) []reconcile.Request {
