...
func (r *KernelCacheReconciler) reconcileCaptureKernelCache(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	selection, err := r.captureNodeGroup(ctx, capture, config, mountType)
	if err != nil {
		...
		if r.Recorder != nil {
			r.Recorder.Event(capture, corev1.EventTypeWarning, selectionErr.Reason, selectionErr.Message)
		}
		return ctrl.Result{}, nil
	}
...
func isCaptureComplete(capture *v1alpha1.KernelCacheCapture) bool {
	if condition := meta.FindStatusCondition(capture.Status.Conditions, kernelCacheCaptureReadyConditionType); condition == nil || condition.Status != metav1.ConditionTrue {
		return false
	}
...
func captureSigningAllowsCache(signing *v1alpha1.KernelCacheSigningStatus) bool {
	...
	if signing.State == v1alpha1.KernelCacheArtifactSecurityStateSkipped {
		return signing.Mode == string(kernelcachetypes.ModeNone)
	}
...
func (r *KernelCacheReconciler) captureNodeGroup(
	...
	if activeSession != nil && strings.TrimSpace(activeSession.RequestedNodeGroup) != "" {
		...
		if err := r.validateNodeGroup(ctx, name, mountType); err != nil {
	...
	if activeSession != nil && activeSession.NodeName != "" {
		...
		if err := r.Get(ctx, client.ObjectKey{Name: activeSession.NodeName}, node); err != nil {
			...
			matches := nodegroup.MatchingGroups(node, groups.Items, mountType)
			switch len(matches) {
			case 1:
...
func (r *KernelCacheReconciler) validateNodeGroup(ctx context.Context, name string, mountType v1alpha1.KernelCacheMountType) error {
	...
	if mountType == v1alpha1.KernelCacheMountTypePVC && !group.HasStorage() {
		return fmt.Errorf("KernelCacheNodeGroup %q does not provide PVC storage", name)
	}
	return nil
}
...
func captureMountType(config *v1beta1.KernelCacheConfig) (v1alpha1.KernelCacheMountType, error) {
	...
	case string(v1alpha1.KernelCacheMountTypePVC):
		return v1alpha1.KernelCacheMountTypePVC, nil
	default:
...
func (r *KernelCacheReconciler) setCaptureKernelCacheRef(ctx context.Context, capture *v1alpha1.KernelCacheCapture, kernelCache *v1alpha1.KernelCache) error {
	if capture.Status.Artifact == nil || !reflect.DeepEqual(*capture.Status.Artifact, kernelCache.Spec.Artifact) {
		return fmt.Errorf("KernelCache %s/%s image does not match KernelCacheCapture %s/%s", kernelCache.Namespace, kernelCache.Name, capture.Namespace, capture.Name)
	}
...
func (r *KernelCacheReconciler) captureToKernelCacheRequests(_ context.Context, obj client.Object) []reconcile.Request {
...
func (r *KernelCacheReconciler) inferenceServiceToKernelCacheRequests(ctx context.Context, obj client.Object) []reconcile.Request {
