...
func TestCaptureConfigFromPodReadsGroupedConfiguration(t *testing.T) {
	want := newTestCaptureConfig("capture", "team", "session-id")
...
func captureConfigEnv(t *testing.T, name, sessionID string) []corev1.EnvVar {
	t.Helper()
	value, err := captureconfig.MarshalCaptureConfig(newTestCaptureConfig(name, "team", sessionID))
	require.NoError(t, err)
	return []corev1.EnvVar{{Name: captureconfig.CaptureConfigEnv, Value: value}}
}

func newTestCaptureConfig(name, namespace, sessionID string) captureconfig.CaptureConfig {
...
func TestRegistryCreatesGeneratedCaptureBeforeActivatingSession(t *testing.T) {
	...
	controller := true
	captureValue, err := captureconfig.MarshalCaptureConfig(newTestCaptureConfig(captureName, namespace, sessionID))
	require.NoError(t, err)
...
func TestEnsureCaptureForPodSkipsWhenCaptureStateExists(t *testing.T) {
	ctx := context.Background()
	scheme := runtime.NewScheme()
	require.NoError(t, corev1.AddToScheme(scheme))
	require.NoError(t, v1alpha1.AddToScheme(scheme))
		...
		t.Run(state, func(t *testing.T) {
			...
			pod := &corev1.Pod{
				...
				Spec: corev1.PodSpec{Containers: []corev1.Container{{
					Name: "mcv",
					Env:  captureConfigEnv(t, "model-capture", "session"),
				}}},
			}
			...
			available, err := reconciler.ensureCaptureForPod(ctx, pod, inferenceService, &v1beta1.KernelCacheConfig{})
			require.NoError(t, err)
...
func TestRegistryIgnoresCapturePodWithoutVerifiedInferenceServiceOwner(t *testing.T) {
	...
	configValue, err := captureconfig.MarshalCaptureConfig(newTestCaptureConfig("model-capture", namespace, "session"))
	require.NoError(t, err)
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
	...
	require.True(t, apierrors.IsNotFound(c.Get(ctx, client.ObjectKey{Namespace: namespace, Name: reporter.ServiceAccount}, &corev1.ServiceAccount{})))
}
...
func TestCaptureSessionBackfillsSchedulingInformation(t *testing.T) {
	...
	pod := &corev1.Pod{
		...
		Spec: corev1.PodSpec{NodeName: "gpu-node", Containers: []corev1.Container{{Name: "mcv", Env: captureConfigEnv(t, "capture", "session")}}},
	}

	inferenceService := &v1beta1.InferenceService{ObjectMeta: metav1.ObjectMeta{Name: "model", Namespace: "team"}}
	selected, terminal, err := r.activateCaptureSession(ctx, pod, inferenceService)
	require.NoError(t, err)
...
func TestCaptureSessionDoesNotResetCompletedCapture(t *testing.T) {
	...
	pod := &corev1.Pod{
		...
		Spec:       corev1.PodSpec{Containers: []corev1.Container{{Name: "mcv", Env: captureConfigEnv(t, "capture", "new-session")}}},
	}
...
// A capture for another InferenceService must not receive a session assignment.
func TestCaptureSessionRejectsMismatchedInferenceService(t *testing.T) {
	ctx := context.Background()
	scheme := runtime.NewScheme()
	require.NoError(t, v1alpha1.AddToScheme(scheme))
	capture := &v1alpha1.KernelCacheCapture{
		ObjectMeta: metav1.ObjectMeta{Name: "capture", Namespace: "team"},
		Spec: v1alpha1.KernelCacheCaptureSpec{SourceRef: v1alpha1.KernelCacheSourceRef{
			Kind: "InferenceService", Name: "other-model",
		}},
	}
	c := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(capture).WithObjects(capture).Build()
	r := &KernelCacheRegistryReconciler{Client: c, Reader: c}
	pod := &corev1.Pod{
		...
		Spec:       corev1.PodSpec{Containers: []corev1.Container{{Name: "mcv", Env: captureConfigEnv(t, "capture", "session")}}},
	}
	inferenceService := &v1beta1.InferenceService{ObjectMeta: metav1.ObjectMeta{Name: "model", Namespace: "team"}}
	selected, terminal, err := r.activateCaptureSession(ctx, pod, inferenceService)
	require.False(t, selected)
	require.False(t, terminal)
	require.EqualError(t, err, "capture source does not match resolved InferenceService")
}
