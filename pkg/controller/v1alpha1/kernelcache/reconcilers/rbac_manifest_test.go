...
func TestLocalModelManagerCanReadWorkloadOwners(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	require.True(t, ok)
	manifestPath := filepath.Join(filepath.Dir(filename), "../../../../../config/rbac/localmodel/role.yaml")
	data, err := os.ReadFile(manifestPath) // #nosec G304 -- the path is resolved from this test source file
	...
	require.True(t, policyRulesAllow(
		...
		"deployments",
		[]string{"get"},
	), "localmodel manager must be able to read Deployment owners")
	require.True(t, policyRulesAllow(
		role.Rules,
		"apps",
		"replicasets",
		[]string{"get"},
	), "localmodel manager must be able to read ReplicaSet owners")
}
