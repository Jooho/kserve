...
const (
	...
	defaultKernelCacheJobTTL      int32 = 600
...
func (r *KernelCacheReconciler) ensureOCIPrefetchJob(
	...
	if err := r.Get(ctx, client.ObjectKey{Name: node.Name}, kernelCacheNode); err == nil {
		...
		if !cacheInfoExists {
			// Wait for the node agent to validate a newly available cache before creating a Job.
			return nil
		}
...
func (r *KernelCacheReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.ServiceAccount{}, handler.EnqueueRequestsFromMapFunc(r.prefetchIdentityRequests), builder.WithPredicates(prefetchIdentityPredicate())).
		Watches(&rbacv1.RoleBinding{}, handler.EnqueueRequestsFromMapFunc(r.prefetchIdentityRequests), builder.WithPredicates(prefetchIdentityPredicate())).
		Complete(r)
}

func prefetchIdentityPredicate() predicate.Predicate {
	return predicate.NewPredicateFuncs(func(obj client.Object) bool {
		return obj.GetName() == kernelCachePrefetchServiceAccount || obj.GetName() == kernelCachePrefetchRoleBinding
	})
}

func (r *KernelCacheReconciler) prefetchIdentityRequests(ctx context.Context, obj client.Object) []reconcile.Request {
