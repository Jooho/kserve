...
import (
	...
	localmodelreconcilers "github.com/kserve/kserve/pkg/controller/v1alpha1/localmodel/reconcilers"
...
const (
	...
	kernelCacheStorageTypeLabel         = "serving.kserve.io/kernel-cache-storage"
	kernelCacheServingStorageType       = "serving"
)
...
func (r *KernelCacheReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if mountType == v1alpha1.KernelCacheMountTypePVC {
		if _, hasPVC := nodeGroup.PersistentVolumeClaimSpec(); !hasPVC {
			return ctrl.Result{}, r.updateStatus(ctx, kernelCache, v1alpha1.KernelCacheStateError, 0, reasonStorageError, "KernelCache PVC mount requires KernelCacheNodeGroup.spec.storage; OCI-only groups must be used with KernelCache.spec.mountType=OCI", mountType)
		}
		if pvSpec, hasPV := nodeGroup.PersistentVolumeSpec(); hasPV {
			if err := validateKernelCachePersistentVolumeSpec(pvSpec); err != nil {
				return ctrl.Result{}, r.updateStatus(ctx, kernelCache, v1alpha1.KernelCacheStateError, 0, reasonStorageError, err.Error(), mountType)
			}
		}
	}
	...
	case v1alpha1.KernelCacheMountTypePVC:
		if err := r.ensureDownloadStorage(ctx, kernelCache, nodeGroup, readyNodes, kernelCacheConfig.JobNamespace); err != nil {
			return ctrl.Result{}, r.updateStatus(ctx, kernelCache, v1alpha1.KernelCacheStateError, nodeCount, reasonStorageError, err.Error(), mountType)
		}
		if err := r.ensureServingStorage(ctx, kernelCache, nodeGroup); err != nil {
			return ctrl.Result{}, r.updateStatus(ctx, kernelCache, v1alpha1.KernelCacheStateError, nodeCount, reasonStorageError, err.Error(), mountType)
		}
		if err := r.ensurePreparationJobs(ctx, kernelCache, nodeGroup, readyNodes, kernelCacheConfig); err != nil {
			return ctrl.Result{}, r.updateStatus(ctx, kernelCache, v1alpha1.KernelCacheStateError, nodeCount, reasonStorageError, err.Error(), mountType)
		}
...
func (r *KernelCacheReconciler) ensureDownloadStorage(
	ctx context.Context,
	kernelCache *v1alpha1.KernelCache,
	nodeGroup *v1alpha1.KernelCacheNodeGroup,
	...
	for i := range readyNodes.Items {
		if err := r.ensureNodeDownloadStorage(ctx, kernelCache, nodeGroup, &readyNodes.Items[i], jobNamespace); err != nil {
			return err
		}
	}
	return nil
...
func (r *KernelCacheReconciler) ensureNodeDownloadStorage(
	ctx context.Context,
	kernelCache *v1alpha1.KernelCache,
	nodeGroup *v1alpha1.KernelCacheNodeGroup,
	node *corev1.Node,
	...
	pvcSpec, hasPVC := nodeGroup.PersistentVolumeClaimSpec()
	if !hasPVC {
		return errors.New("KernelCacheNodeGroup.spec.storage is required for PVC mount")
	}
	pvSpec, hasPV := nodeGroup.PersistentVolumeSpec()
	if hasPV {
		if err := validateKernelCachePersistentVolumeSpec(pvSpec); err != nil {
			return err
		}
	}
	resourceName := downloadStorageName(kernelCache, nodeGroup, node)
	...
	if hasPV {
		if node != nil {
			pvSpec.NodeAffinity = nodeAffinityForNode(*node)
		}
		persistentVolume := &corev1.PersistentVolume{
			ObjectMeta: metav1.ObjectMeta{Name: resourceName, Labels: labels},
			Spec:       pvSpec,
		}
		if err := r.ensurePersistentVolume(ctx, persistentVolume); err != nil {
			return err
		}
		pvcSpec.VolumeName = resourceName
	}

	persistentVolumeClaim := &corev1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{Name: resourceName, Namespace: jobNamespace, Labels: labels},
		Spec:       pvcSpec,
	}
	return r.ensurePersistentVolumeClaim(ctx, persistentVolumeClaim)
}

func (r *KernelCacheReconciler) ensureServingStorage(
	ctx context.Context,
	kernelCache *v1alpha1.KernelCache,
	nodeGroup *v1alpha1.KernelCacheNodeGroup,
) error {
	if r.Clientset == nil {
		return errors.New("kubernetes clientset is required for KernelCache serving storage")
	}
	pvcSpec, hasPVC := nodeGroup.PersistentVolumeClaimSpec()
	if !hasPVC {
		return errors.New("KernelCacheNodeGroup.spec.storage is required for PVC mount")
	...
	pvcName := servingStorageName(kernelCache, nodeGroup)
	labels := kernelCacheLabels(kernelCache)
	labels[kernelCacheStorageTypeLabel] = kernelCacheServingStorageType

	if hasPV {
		pvName := servingPVName(kernelCache, nodeGroup)
		pvSpec.ClaimRef = nil
		persistentVolume := corev1.PersistentVolume{
		...
		if err := localmodelreconcilers.CreatePV(ctx, r.Clientset, r.Scheme, r.Log, persistentVolume, nil, nil); err != nil {
			return err
		}
	...
	return localmodelreconcilers.CreatePVC(ctx, r.Clientset, r.Scheme, r.Log, persistentVolumeClaim, kernelCache.Namespace, nil, nil)
}

func validateKernelCachePersistentVolumeSpec(spec corev1.PersistentVolumeSpec) error {
	if spec.HostPath != nil {
		return errors.New("hostPath is not supported for KernelCache storage")
	}
	return nil
}

func (r *KernelCacheReconciler) ensurePreparationJobs(
	ctx context.Context,
	kernelCache *v1alpha1.KernelCache,
	nodeGroup *v1alpha1.KernelCacheNodeGroup,
	readyNodes *corev1.NodeList,
	...
	if hasAccessMode(pvcSpec.AccessModes, corev1.ReadWriteMany) {
		node := &readyNodes.Items[0]
		return r.ensurePreparationJob(ctx, kernelCache, nodeGroup, node, config, false)
	}

	for i := range readyNodes.Items {
		if err := r.ensurePreparationJob(ctx, kernelCache, nodeGroup, &readyNodes.Items[i], config, true); err != nil {
			return err
		}
	}
	return nil
...
func (r *KernelCacheReconciler) ensurePreparationJob(
	ctx context.Context,
	kernelCache *v1alpha1.KernelCache,
	nodeGroup *v1alpha1.KernelCacheNodeGroup,
	node *corev1.Node,
	...
	jobName := preparationJobName(kernelCache, node, nodeSpecific)
	job := &batchv1.Job{}
	...
	pvcName := downloadStorageName(kernelCache, nodeGroup, node)
	job = &batchv1.Job{
		ObjectMeta: metav1.ObjectMeta{
			Name:      jobName,
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						Name:            "kernel-cache-preparer",
						Image:           config.MCVImage,
						ImagePullPolicy: corev1.PullIfNotPresent,
						Command:         []string{"/mcv"},
						Args: []string{
							"--extract",
							"--skip-precheck",
							"--image", kernelCache.Spec.Artifact.ImageReference,
							"--dir", "/mnt/kernel-cache",
						},
	...
	if nodeSpecific {
		job.Spec.Template.Spec.NodeSelector = map[string]string{"kubernetes.io/hostname": nodeHostname(node)}
	}

	if err := podconfig.ApplyRegistry(&job.Spec.Template.Spec, &job.Spec.Template.Spec.Containers[0], config.Registry); err != nil {
		return err
	}
	automount := false
...
func (r *KernelCacheReconciler) ensureOCIPrefetchJob(
	...
	if err := r.Get(ctx, client.ObjectKey{Name: node.Name}, kernelCacheNode); err == nil {
		info := kernelCacheNode.Status.CacheStatus[kernelCache.Namespace+"/"+kernelCache.Name]
		if info.State == v1alpha1.KernelCacheNodePreparationStateReady &&
			info.ImageReference == kernelCache.Spec.Artifact.ImageReference &&
			info.Footprints == kernelCache.Spec.Artifact.Identity.Footprints {
			return nil
		}
	} else if !apierrors.IsNotFound(err) {
		return err
...
func hasAccessMode(accessModes []corev1.PersistentVolumeAccessMode, wanted corev1.PersistentVolumeAccessMode) bool {
	for _, accessMode := range accessModes {
		if accessMode == wanted {
			return true
		}
	}
	return false
...
func downloadStorageName(kernelCache *v1alpha1.KernelCache, nodeGroup *v1alpha1.KernelCacheNodeGroup, node *corev1.Node) string {
	name := fmt.Sprintf("%s-%s-%s", kernelCache.Namespace, kernelCache.Name, nodeGroup.Name)
	if node != nil {
		name += "-" + node.Name
	}
...
func servingStorageName(kernelCache *v1alpha1.KernelCache, nodeGroup *v1alpha1.KernelCacheNodeGroup) string {
	return fmt.Sprintf("%s-%s", kernelCache.Name, nodeGroup.Name)
}

func servingPVName(kernelCache *v1alpha1.KernelCache, nodeGroup *v1alpha1.KernelCacheNodeGroup) string {
	return fmt.Sprintf("%s-%s", servingStorageName(kernelCache, nodeGroup), kernelCache.Namespace)
}

func nodeAffinityForNode(node corev1.Node) *corev1.VolumeNodeAffinity {
...
func (r *KernelCacheReconciler) ensurePersistentVolume(ctx context.Context, desired *corev1.PersistentVolume) error {
	current := &corev1.PersistentVolume{}
	if err := r.Get(ctx, client.ObjectKey{Name: desired.Name}, current); err == nil {
		if current.Labels[kernelCacheNameLabel] != desired.Labels[kernelCacheNameLabel] || current.Labels[kernelCacheNamespaceLabel] != desired.Labels[kernelCacheNamespaceLabel] {
			return fmt.Errorf("persistent volume %q already exists and is not managed by this KernelCache", desired.Name)
		}
		return nil
	} else if !apierrors.IsNotFound(err) {
		return err
...
func (r *KernelCacheReconciler) ensurePersistentVolumeClaim(ctx context.Context, desired *corev1.PersistentVolumeClaim) error {
	current := &corev1.PersistentVolumeClaim{}
	if err := r.Get(ctx, client.ObjectKey{Name: desired.Name, Namespace: desired.Namespace}, current); err == nil {
		if current.Labels[kernelCacheNameLabel] != desired.Labels[kernelCacheNameLabel] || current.Labels[kernelCacheNamespaceLabel] != desired.Labels[kernelCacheNamespaceLabel] {
			return fmt.Errorf("persistent volume claim %q already exists and is not managed by this KernelCache", desired.Name)
		}
		return nil
	} else if !apierrors.IsNotFound(err) {
		return err
...
func (r *KernelCacheReconciler) SetupWithManager(mgr ctrl.Manager) error {
	if r.Recorder == nil {
		r.Recorder = mgr.GetEventRecorderFor("kernelcache-controller")
	}
