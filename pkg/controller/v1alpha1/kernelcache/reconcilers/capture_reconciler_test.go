...
func TestCleanupCaptureIdentitiesRevokesRegistryAccess(t *testing.T) {
	...
	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{
		...
		Annotations: map[string]string{
			registryauth.AccessSecretAnnotation: "mcv-registry-capture",
		},
	}}
	secret := &corev1.Secret{ObjectMeta: metav1.ObjectMeta{
		Name:            "mcv-registry-capture",
	...
	capture := &v1alpha1.KernelCacheCapture{
		...
		Status: v1alpha1.KernelCacheCaptureStatus{
			Phase:         v1alpha1.KernelCacheCapturePhaseComplete,
			ActiveSession: &v1alpha1.KernelCacheCaptureSession{PodName: pod.Name},
		},
	}
	k8sClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(pod, secret).Build()
	reconciler := &KernelCacheCaptureReconciler{Client: k8sClient, Reader: k8sClient}

	require.NoError(t, reconciler.cleanupCaptureIdentities(t.Context(), capture))

	remaining := &corev1.Secret{}
	err := k8sClient.Get(t.Context(), client.ObjectKeyFromObject(secret), remaining)
	require.True(t, apierrors.IsNotFound(err))
}

func TestCleanupCaptureIdentitiesSkipsRegistryAccessForNonTerminalCapture(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, corev1.AddToScheme(scheme))
	require.NoError(t, v1alpha1.AddToScheme(scheme))
