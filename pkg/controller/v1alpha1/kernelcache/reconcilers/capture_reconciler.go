...
func (r *KernelCacheCaptureReconciler) capturePod(ctx context.Context, capture *v1alpha1.KernelCacheCapture, result map[string]string) (*corev1.Pod, error) {
	...
	if pod.Labels[appsv1.DefaultDeploymentUniqueLabelKey] != "" && capture.Labels[constants.KernelCacheCaptureGeneratedLabelKey] == "true" {
		revision, err := workload.ResolveInferenceServiceRevision(ctx, reader, pod)
		if err != nil {
			return nil, err
		}
	...
	} else if resolved, err := workload.ResolveInferenceService(ctx, reader, pod); err != nil {
		return nil, err
