...
func (r *KernelCacheRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if isCapturePod {
		...
		selected, terminal, err := r.activateCaptureSession(ctx, capturePod, inferenceService)
		if err != nil {
			return ctrl.Result{}, err
		}
		if !selected {
...
func (r *KernelCacheRegistryReconciler) activateCaptureSession(ctx context.Context, pod *corev1.Pod, inferenceService *v1beta1.InferenceService) (bool, bool, error) {
	captureConfig, found, err := captureConfigFromPod(pod)
	if err != nil {
	...
	if inferenceService == nil {
		return false, false, errors.New("capture Pod has no resolved InferenceService")
	}
	name := captureConfig.Capture.Name
	...
	err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if !captureSourceMatches(capture, inferenceService) {
			return errors.New("capture source does not match resolved InferenceService")
		}
...
func (r *KernelCacheRegistryReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	b := ctrl.NewControllerManagedBy(mgr).Named("kernelcache-registry").
		...
		Watches(&corev1.Pod{}, handler.EnqueueRequestsFromMapFunc(func(_ context.Context, obj client.Object) []reconcile.Request {
			...
			if !ok || obj.GetLabels()[constants.InferenceServicePodLabelKey] == "" || !hasMCVContainer(pod) {
				return nil
			}
