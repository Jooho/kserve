...
func (r *KernelCacheRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.cleanupLegacySingletons(ctx, ns); err != nil {
		return ctrl.Result{}, err
	}
	...
	if err := r.Reader.Get(ctx, client.ObjectKey{Namespace: ns, Name: req.Name}, capturePod); err == nil {
		...
		if found {
			inferenceService, err = workload.ResolveInferenceService(ctx, r.Reader, capturePod)
...
func (r *KernelCacheRegistryReconciler) ensureCaptureForPod(ctx context.Context, pod *corev1.Pod, inferenceService *v1beta1.InferenceService, cfg *v1beta1.KernelCacheConfig) (bool, error) {
	...
	revision, resolveErr := workload.ResolveInferenceServiceRevision(ctx, reader, pod)
	if resolveErr != nil {
...
func (r *KernelCacheRegistryReconciler) ensureReporterIdentityForCapture(ctx context.Context, capture *v1alpha1.KernelCacheCapture) error {
...
func (r *KernelCacheRegistryReconciler) ensureTokenRequesterBinding(ctx context.Context, namespace string) error {
	binding := &rbacv1.RoleBinding{
		ObjectMeta: metav1.ObjectMeta{Name: registryauth.TokenRequesterRole, Namespace: namespace},
...
func (r *KernelCacheRegistryReconciler) cleanupLegacySingletons(ctx context.Context, namespace string) error {
	resources := []struct {
		object client.Object
		label  string
	}{
		{object: &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: "kernel-cache-reporter", Namespace: namespace}}, label: reporter.ManagedLabel},
		{object: &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "kernel-cache-reporter", Namespace: namespace}}, label: reporter.ManagedLabel},
		{object: &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: "kernel-cache-pusher", Namespace: namespace}}, label: registryauth.ManagedLabel},
		{object: &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "kernel-cache-pusher", Namespace: namespace}}, label: registryauth.ManagedLabel},
	}
	for _, resource := range resources {
		current := resource.object.DeepCopyObject().(client.Object)
		if err := r.Get(ctx, client.ObjectKeyFromObject(resource.object), current); err != nil {
			if apierrors.IsNotFound(err) {
				continue
			}
			return err
		}
		if current.GetLabels()[resource.label] != "true" || metav1.GetControllerOf(current) != nil {
			continue
		}
		uid := current.GetUID()
		resourceVersion := current.GetResourceVersion()
		if err := r.Delete(ctx, current, client.Preconditions{UID: &uid, ResourceVersion: &resourceVersion}); client.IgnoreNotFound(err) != nil {
			return err
		}
	}
	return nil
...
func (r *KernelCacheRegistryReconciler) ensureManagedServiceAccountWithOwner(ctx context.Context, namespace, name, label string, owner *metav1.OwnerReference) error {
	automount := false
...
func (r *KernelCacheRegistryReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	b := ctrl.NewControllerManagedBy(mgr).Named("kernelcache-registry").
		...
		Watches(&rbacv1.RoleBinding{}, handler.EnqueueRequestsFromMapFunc(r.tokenRequesterBindingRequests), builder.WithPredicates(predicate.NewPredicateFuncs(func(obj client.Object) bool {
			return obj.GetName() == registryauth.TokenRequesterRole
		}))).
...
func (r *KernelCacheRegistryReconciler) tokenRequesterBindingRequests(ctx context.Context, obj client.Object) []reconcile.Request {
	services := &v1beta1.InferenceServiceList{}
