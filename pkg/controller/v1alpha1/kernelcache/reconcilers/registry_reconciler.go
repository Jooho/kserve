...
func (r *KernelCacheRegistryReconciler) ensurePusherIdentityForCapture(ctx context.Context, capture *v1alpha1.KernelCacheCapture, config v1beta1.KernelCacheRegistryConfig) error {
	...
	return r.reconcilePusherRoleBindingForActiveCapture(ctx, capture, binding)
}

// reconcilePusherRoleBindingForActiveCapture keeps the current RoleRef while a
// capture is running so a ConfigMap update cannot revoke the active pusher.
// A new capture receives the current configured RoleRef when its binding is created.
func (r *KernelCacheRegistryReconciler) reconcilePusherRoleBindingForActiveCapture(ctx context.Context, capture *v1alpha1.KernelCacheCapture, desired *rbacv1.RoleBinding) error {
...
func (r *KernelCacheRegistryReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	b := ctrl.NewControllerManagedBy(mgr).Named("kernelcache-registry").
		...
		Watches(&corev1.ConfigMap{}, handler.EnqueueRequestsFromMapFunc(r.registryConfigRequests), builder.WithPredicates(predicate.NewPredicateFuncs(isInferenceServiceConfigMap))).
