...
func TestKernelCacheReconcilerCreatesKernelCacheFromCompletedCapture(t *testing.T) {
	...
	k8sClient := fake.NewClientBuilder().
		...
		Build()
	reconciler := &KernelCacheReconciler{Client: k8sClient, Log: logr.Discard(), Scheme: scheme}
	...
	requests := reconciler.captureToKernelCacheRequests(context.Background(), secondCapture)
	...
	if _, err := reconciler.reconcileCaptureKernelCache(context.Background(), requests[0]); err != nil {
		t.Fatal(err)
	}
...
func TestKernelCacheReconcilerAutomaticallySelectsUniqueNodeGroup(t *testing.T) {
	reconciler, k8sClient, capture, request := newCaptureNodeGroupHarness(t,
		v1alpha1.KernelCacheMountTypeOCI,
		"gpu-node",
		"",
		"fallback",
...
func TestKernelCacheReconcilerPVCMatchingRequiresStorage(t *testing.T) {
	reconciler, k8sClient, _, request := newCaptureNodeGroupHarness(t,
		v1alpha1.KernelCacheMountTypePVC,
		"gpu-node",
		"",
		"",
		[]v1alpha1.KernelCacheNodeGroup{
			{ObjectMeta: metav1.ObjectMeta{Name: "oci-only"}, Spec: v1alpha1.KernelCacheNodeGroupSpec{NodeSelector: map[string]string{"role": "gpu"}}},
			{ObjectMeta: metav1.ObjectMeta{Name: "pvc"}, Spec: v1alpha1.KernelCacheNodeGroupSpec{
	...
	cache := &v1alpha1.KernelCache{}
	require.NoError(t, k8sClient.Get(t.Context(), request.NamespacedName, cache))
	require.Equal(t, "pvc", cache.Spec.NodeGroupRef.Name)
}
...
func TestNodeGroupChangeRequeuesUnboundCompletedCapture(t *testing.T) {
	...
	requests := reconciler.nodeGroupToKernelCaches(t.Context(), &v1alpha1.KernelCacheNodeGroup{
...
func TestNodeChangeRequeuesCaptureAssignedToThatNode(t *testing.T) {
	...
	requests := reconciler.nodeToKernelCaches(t.Context(), &corev1.Node{
...
func newCaptureNodeGroupHarness(
	...
	configMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: constants.InferenceServiceConfigMapName, Namespace: constants.KServeNamespace},
		Data:       map[string]string{"kernelcache": `{"enabled":true,"defaultMountType":"` + string(mountType) + `","defaultNodeGroup":"` + defaultNodeGroup + `"}`},
	}
	...
	reconciler := &KernelCacheReconciler{Client: k8sClient, Reader: k8sClient, Log: logr.Discard(), Scheme: scheme}
