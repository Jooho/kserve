...
func (r *KernelCacheNodeReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.Node{}, handler.EnqueueRequestsFromMapFunc(r.nodeToNodeGroupRequests), builder.WithPredicates(nodeGroupNodePredicate())).
...
// nodeGroupNodePredicate accepts Node lifecycle and label changes that affect group membership.
func nodeGroupNodePredicate() predicate.Predicate {
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			return oldOK && newOK && !reflect.DeepEqual(oldNode.Labels, newNode.Labels)
		},
...
func (r *KernelCacheNodeReconciler) inferenceServiceConfigMapPredicate() predicate.Predicate {
	return predicate.NewPredicateFuncs(isInferenceServiceConfigMap)
}
