...
func (r *KernelCacheNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, group); err != nil {
		if apierrors.IsNotFound(err) {
			return ctrl.Result{}, r.reconcileAgentDaemonSet(ctx)
		}
	...
	readyNodes, _, err := nodegroup.GetNodes(ctx, group, r.Client)
	if err != nil {
		return ctrl.Result{}, err
	}

	for i := range readyNodes.Items {
		node := &readyNodes.Items[i]
		kernelCacheNode := &v1alpha1.KernelCacheNode{}
		err := r.Get(ctx, client.ObjectKey{Name: node.Name}, kernelCacheNode)
		if err == nil {
			continue
		}
		if !apierrors.IsNotFound(err) {
