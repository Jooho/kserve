...
func (m *PodMutator) findKernelCache(ctx context.Context, pod *corev1.Pod) (*v1alpha1.KernelCache, bool, error) {
	...
	if selection, found, err := loadExactKernelCache(ctx, reader, pod, requested, candidates); err != nil {
		return nil, false, err
	} else if found {
	...
	caches, err := loadWeightedKernelCaches(ctx, reader, pod, candidates)
	if err != nil {
		return nil, false, err
	}
	selection := selectWeightedMatch(requested, caches)
	if selection == nil {
		return nil, false, nil
	}
	logger.Info("Matched KernelCache", "kernelCache", client.ObjectKeyFromObject(selection.cache), "matchType", selection.matchType, "score", selection.score)
...
func loadExactKernelCache(
	...
	for _, candidate := range candidates {
		...
		if found {
			return &kernelCacheSelection{cache: cache, matchType: kernelCacheMatchWorkload, score: 100}, true, nil
		}
	}
	for _, candidate := range candidates {
		...
		if found {
			return &kernelCacheSelection{cache: cache, matchType: kernelCacheMatchCompatibility, score: 100}, true, nil
		}
	}
	return nil, false, nil
}

func loadWeightedKernelCaches(
	ctx context.Context,
	reader client.Reader,
	pod *corev1.Pod,
	candidates []kernelCacheNodeCandidate,
) ([]*v1alpha1.KernelCache, error) {
	caches := make([]*v1alpha1.KernelCache, 0, len(candidates))
	for _, candidate := range candidates {
		cache, found, err := loadKernelCacheCandidate(ctx, reader, pod, candidate)
		if err != nil {
			return nil, err
