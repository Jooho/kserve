...
// Builds the KernelCache identity from the Pod workload and runtime container.
func identityForPod(pod *corev1.Pod) (v1alpha1.KernelCacheIdentity, error) {
	...
	// Todo: for llmisvc, need to use another way to find model uri
	identityFactors := buildRuntimeIdentityFactors(
		container,
		pod.Annotations[constants.StorageInitializerSourceUriInternalAnnotationKey],
	)
...
// Extracts the workload kind and name from the Pod labels.
...
// Finds the runtime container associated with the workload kind.
...
// Builds runtime identity factors from a container and model URI.
