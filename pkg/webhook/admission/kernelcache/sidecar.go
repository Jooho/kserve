...
func (m *PodMutator) injectMCVSidecar(ctx context.Context, pod *corev1.Pod, cfg *v1beta1.KernelCacheConfig) error {
	...
	defaultCaptureName := constants.KernelCacheCaptureName(workload.Name)
	...
	capture, err := captureForSidecar(ctx, reader, pod.Namespace, workload)
	if err != nil {
		return err
	}
	if capture != nil {
...
func captureForSidecar(
	...
	for index := range captures.Items {
		...
		sourceMatches := capture.Spec.SourceRef.Kind == workload.Kind && capture.Spec.SourceRef.Name == workload.Name
		if sourceMatches {
...
func defaultCachePaths(container *corev1.Container) ([]v1alpha1.KernelCachePath, error) {
	paths := []v1alpha1.KernelCachePath{
		{ContainerName: container.Name, ContainerPath: "/root/.cache/vllm"},
