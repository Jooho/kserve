...
func TestInjectSidecar(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "test", Labels: map[string]string{constants.InferenceServicePodLabelKey: "qwen", "pod-template-hash": "669889f77f"}}, Spec: corev1.PodSpec{ServiceAccountName: "runtime", Containers: []corev1.Container{{Name: "kserve-container", Env: []corev1.EnvVar{{Name: "VLLM_CACHE_ROOT", Value: "/cache/vllm"}}, ReadinessProbe: &corev1.Probe{ProbeHandler: corev1.ProbeHandler{HTTPGet: &corev1.HTTPGetAction{Path: "/health", Port: intstr.FromInt(8080)}}, PeriodSeconds: 5, TimeoutSeconds: 2}}}}}
...
func sidecarTestPod() *corev1.Pod {
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Namespace: "test", Labels: map[string]string{constants.InferenceServicePodLabelKey: "qwen", "pod-template-hash": "669889f77f"}},
		Spec: corev1.PodSpec{Containers: []corev1.Container{{
			Name: "kserve-container",
...
func TestTerminalCaptureSkipsCaptureSidecar(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, v1alpha1.AddToScheme(scheme))
	capture := &v1alpha1.KernelCacheCapture{
		ObjectMeta: metav1.ObjectMeta{Name: "qwen-kcc-669889f77f", Namespace: "test"},
...
func TestSkipSidecarForTerminalCapture(t *testing.T) {
	for _, phase := range []v1alpha1.KernelCacheCapturePhase{
		...
		t.Run(string(phase), func(t *testing.T) {
			require.True(t, skipSidecarForTerminalCapture(&v1alpha1.KernelCacheCapture{
