...
func (m *PodMutator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	// injector := podadmission.NewKernelCacheInjector(m.Client)
	// if err := injector.Inject(ctx, pod, v1alpha1.KernelCacheMountType(kernelCacheConfig.DefaultMountType)); err != nil {
	// 	logger.Error(err, "Failed to inject KernelCache", "pod", pod.Name, "namespace", req.Namespace)
	// 	return admission.Errored(http.StatusInternalServerError, err)
	// }
	...
	if _, err := m.injectKernelCacheConsumer(ctx, pod, kernelCacheConfig); err != nil {
		logger.Error(err, "Failed to mount KernelCache")
		return admission.Errored(http.StatusInternalServerError, err)
	}

	if sidecarInjection {
		if err := m.injectSidecar(ctx, pod, kernelCacheConfig); err != nil {
