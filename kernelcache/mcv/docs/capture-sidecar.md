...
variables. These variables are required for every capture sidecar.

The webhook also sets `MCV_CAPTURE_MODE=true` so the image entrypoint starts
the capture orchestrator instead of the regular MCV CLI.
