...
def capture_config():
    ...
    try:
        cache_paths = json.loads(os.environ.get("MCV_CACHE_PATHS", "[]"))
    except json.JSONDecodeError as error:
        raise CaptureConfigurationError("MCV_CACHE_PATHS must contain valid JSON") from error
    if not isinstance(cache_paths, list):
        raise CaptureConfigurationError("MCV_CACHE_PATHS must contain a JSON array")
    return validate_capture_config({
        "version": 1,
        "cacheDir": os.environ.get("MCV_CACHE_DIR", ""),
        "targetImage": os.environ.get("MCV_TARGET_IMAGE", ""),
        "capture": {
            "name": os.environ.get("MCV_CAPTURE_NAME", ""),
            "namespace": os.environ.get("MCV_CAPTURE_NAMESPACE", ""),
            "sessionID": os.environ.get("MCV_CAPTURE_SESSION_ID", ""),
        },
...
def readiness_config():
    ...
    if os.environ.get("MCV_READINESS_PROBE_TYPE") != "httpGet":
        raise CaptureConfigurationError("MCV_READINESS_PROBE_TYPE must be httpGet")
    ...
    if not port:
        raise CaptureConfigurationError("MCV_READINESS_PROBE_PORT is required")
    return validate_readiness_config({
...
def reporter_request(status):
    ...
    capture = config.get("capture", {})
    namespace = os.environ.get("MCV_CAPTURE_NAMESPACE", "")
    namespace = capture.get("namespace", namespace)
    ...
    session_id = capture.get("sessionID", os.environ.get("MCV_CAPTURE_SESSION_ID", ""))
...
def capture_result():
    ...
    result["captureSessionID"] = config.get("capture", {}).get(
        "sessionID", os.environ.get("MCV_CAPTURE_SESSION_ID", "")
    )
...
def failure_result(reason, message):
    ...
    return {
        ...
        "captureSessionID": config.get("capture", {}).get(
            "sessionID", os.environ.get("MCV_CAPTURE_SESSION_ID", "")
        ),
    }
...
def wait_for_registry_access():
    ...
    if access_file or os.environ.get("MCV_REGISTRY_AUTH_REQUIRED", "false") == "true":
...
def main():
    ...
    try:
        ...
        capture_session_id = config.get("capture", {}).get(
            "sessionID", os.environ.get("MCV_CAPTURE_SESSION_ID", "")
        )
        report({
