...
RELEASE_JSON=$(gh release view "${VERSION}" --repo "${REPO}" --json isDraft,isPrerelease,url 2>/dev/null || echo "")
if [[ -z "$RELEASE_JSON" ]]; then
    check_fail "GitHub Release ${VERSION} not found" "Run publish-release.sh"
else
    IS_DRAFT=$(echo "$RELEASE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin)['isDraft'])" 2>/dev/null)
    IS_PRERELEASE=$(echo "$RELEASE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin)['isPrerelease'])" 2>/dev/null)
    RELEASE_URL=$(echo "$RELEASE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin)['url'])" 2>/dev/null)
