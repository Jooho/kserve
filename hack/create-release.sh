...
validate_kserve_deps() {
    ...
    if [[ "$current_version" != "$VERSION" ]]; then
        ...
        echo "  ./hack/prepare-for-release.sh <prior_version> <new_version>"
        exit 1
    fi
...
execute_release() {
    ...
    if [[ "${GITHUB_ACTIONS:-false}" == "true" ]]; then
        ...
        echo "  1. GitHub Actions workflow will create the release"
        echo "  2. Or manually create release on GitHub:"
        echo "     https://github.com/kserve/kserve/releases/new?tag=$VERSION"
    fi
    echo ""
}
