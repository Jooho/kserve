...
def build_uninstall_calls(components: list[dict[str, Any]]) -> str:
...
def generate_script_content(definition_file: Path,
    ...
    install_calls_str = build_install_calls(components, config["global_env"])
    uninstall_calls = build_uninstall_calls(components)
