...
def test_parse_definition_release_independent_from_embed_manifests(tmp_path):
    """Test that RELEASE and EMBED_MANIFESTS are independent."""
    definition_content = """
FILE_NAME: test-install
RELEASE: true
COMPONENTS:
...
"""
    ...
    # RELEASE and EMBED_MANIFESTS are independent
    assert result["release"] is True
    assert result["embed_manifests"] is False  # Defaults to False


def test_parse_definition_embed_manifests_and_release_separate(tmp_path):
    """Test that EMBED_MANIFESTS and RELEASE can be set independently."""
    definition_content = """
FILE_NAME: test-install
EMBED_MANIFESTS: true
RELEASE: false
