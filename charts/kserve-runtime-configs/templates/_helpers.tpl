  ...
  {{- if hasKey $base $key -}}
    ...
    {{- else if and (kindIs "slice" $value) (kindIs "slice" $baseValue) -}}
      ...
      {{- if $canMergeByName -}}
        ...
        {{- $mergedResult := include "kserve-runtime-configs.mergeArrayByName" (list $baseValue $value) | fromYaml -}}
...
Usage: include "kserve-runtime-configs.mergeArrayByName" (list $baseArray $patchArray) | fromYaml
...
*/}}
{{- define "kserve-runtime-configs.mergeArrayByName" -}}
{{- $baseArray := index . 0 -}}
{{- $patchArray := index . 1 -}}
{{- $processedNames := dict -}}
...
{{- range $baseItem := $baseArray -}}
  ...
  {{- range $patchItem := $patchArray -}}
    {{- if eq $patchItem.name $name -}}
      ...
      {{- $merged := include "kserve-runtime-configs.deepMerge" (list $baseItem $patchItem) | fromYaml -}}
